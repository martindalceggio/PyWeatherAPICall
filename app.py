...
def get_forecast_weather(city):
    url = f"{api_endpoint}forecast/daily?city={city}&key={api_key}"
    response = requests.get(url)
    data = response.json()
    return data
...
def index():
    ...
    return render_template("index.html", current_weather=current_weather, forecast_weather=forecast_weather)
